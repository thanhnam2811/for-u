// ── Lines 98 — Save / Checkpoint / Leaderboard ──
...
export function autoSave() {
  ...
  if (user) {
    ...
    setDoc(userDocRef, {
      saveGame98: stringified,
      updatedAt: Date.now()
    }, { merge: true }).catch(err => console.error("Firestore autoSave98 error:", err));
  }
}
...
export function clearSave() {
  ...
  if (user) {
    ...
    setDoc(userDocRef, {
      saveGame98: null,
      updatedAt: Date.now()
    }, { merge: true }).catch(err => console.error("Firestore clearSave98 error:", err));
  }
}
...
export function saveCheckpoint(slot) {
  ...
  if (user) {
    ...
    setDoc(userDocRef, {
      ...
      updatedAt: Date.now()
    }, { merge: true }).catch(err => console.error("Firestore saveCheckpoint98 error:", err));
  }
}
...
export function deleteCheckpoint(slot) {
  ...
  if (user) {
    ...
    setDoc(userDocRef, {
      ...
      updatedAt: Date.now()
    }, { merge: true }).catch(err => console.error("Firestore deleteCheckpoint98 error:", err));
  }
}
...
export async function getGlobalLeaderboard() {
  try {
    ...
    snap.forEach(docSnap => {
      ...
      if (d.highScore98 && d.highScore98 > 0) {
        list.push({
          displayName: d.displayName || "Người chơi ẩn danh",
          photoURL: d.photoURL || "",
          score: d.highScore98,
          date: d.updatedAt || Date.now(),
          moves: d.highScoreMoves98 || 0
        });
      }
    });
    return list;
  } catch (err) {
    console.error("Firestore getGlobalLeaderboard98 error:", err);
  ...
  try {
    ...
    if (docSnap.exists()) {
      ...
      if (justLoggedIn) {
        ...
        const hasCloudActive = !!cloudData.saveGame98;
        const hasLocalHighScore = localHighScore > 0;
        const hasCloudHighScore = (cloudData.highScore98 || 0) > 0;

        if ((hasLocalActive && hasCloudActive) || (hasLocalHighScore && hasCloudHighScore && localHighScore !== cloudData.highScore98)) {
          return new Promise((resolve) => {
            ...
            let cloudInfo = `Kỷ lục: ${cloudData.highScore98 || 0}đ`;
            if (cloudData.saveGame98) {
              try {
                const s = JSON.parse(cloudData.saveGame98);
      ...
      if (localSaveRaw) {
        initialData.saveGame98 = localSaveRaw;
      }
...
async function applyCloudData(cloudData) {
  const cloudHighScore = cloudData.highScore98 || 0;
  localStorage.setItem(LS_HIGH, cloudHighScore);
  state.highScore = cloudHighScore;

  if (cloudData.saveGame98) {
    localStorage.setItem(LS_SAVE, cloudData.saveGame98);
  } else {
    localStorage.removeItem(LS_SAVE);
  }

  const cloudCheckpoints = cloudData.checkpoints98 || {};
  for (let i = 0; i < 3; i++) {
...
async function autoMergeUserProgress(userDocRef, cloudData, localHighScore, localSaveRaw, localCheckpoints, user) {
  ...
  // 2. Save Game Sync
  const cloudSave = cloudData.saveGame98 || null;
  let cloudSaveTime = 0;
  if (cloudSave) {
  ...
  if (needsUpload) {
    ...
    if (localHighScore > cloudHighScore) {
      uploadData.highScore98 = localHighScore;
    }
    if (localSaveRaw) {
      uploadData.saveGame98 = localSaveRaw;
    }
